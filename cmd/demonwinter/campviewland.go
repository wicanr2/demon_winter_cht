...
func (a *app) updateViewLand() error {
	...
	if v.member >= 0 {
		...
		case inpututil.IsKeyJustPressed(ebiten.KeyEnter):
			...
			v.member = -1
			a.camp.message = fmt.Sprintf(a.tr.UI("viewland.climbed"), c.Name)
		}
		return nil
	}
...
func (a *app) drawViewLand(dst *ebiten.Image, line func(string)) {
	...
	right(a.tr.UI("viewland.header"))
	right("")
	right(fmt.Sprintf(a.tr.UI("viewland.coord"), v.x, v.y))
	right(fmt.Sprintf(a.tr.UI("viewland.mapid"), a.mapID))
	right("")
	right(a.tr.UI("viewland.keys_look"))
	right(a.tr.UI("viewland.keys_down"))
}
