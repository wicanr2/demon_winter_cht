...
func (a *app) openManual() {
	if a.manual.Len() == 0 {
		a.message = a.tr.UI("manual.unavailable")
		return
	}
...
func (a *app) drawManual(dst *ebiten.Image) {
	...
	if !m.reading {
		...
		line("")
		line(a.tr.UI("manual.toc.keys"))
		return
	}

	sec := a.manual.At(m.section)
	line(fmt.Sprintf(a.tr.UI("manual.reading.header"), sec.Title))
	line("")
	...
	if len(sec.Lines) > manualLinesPerPage {
		line(fmt.Sprintf(a.tr.UI("manual.reading.scroll"),
			m.top+1, min(m.top+manualLinesPerPage, len(sec.Lines)), len(sec.Lines)))
	} else {
		line(a.tr.UI("manual.reading.back"))
	}
}
