...
func (a *app) openWorldTemple(x, y int) {
	...
	if deity == 0 {
		...
		a.message = fmt.Sprintf(a.tr.UI("site.temple.gap"), x, y)
		return
	}
	name := fmt.Sprintf(a.tr.UI("site.temple.name"), a.deityName(deity))
	a.openWorldSite(name, game.FacilityChurch, gamedata.TownFacilities{Church: deity})
	a.message = a.tr.UI("site.enter") + name
}
...
func (a *app) openWorldCollege(x, y int) {
	...
	if !ok {
		a.message = fmt.Sprintf(a.tr.UI("site.college.gap"), x, y)
		return
	}
	name := fmt.Sprintf(a.tr.UI("site.college.name"), a.skillName(skill))
