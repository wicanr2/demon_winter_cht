...
> **要深入的話，先讀 [`CONTEXT.md`](CONTEXT.md)** —— 全專案的單一入口，
> 含現況一覽、完整文件索引、術語表、以及已被推翻的斷言清單。
> 逆向筆記累積已超過 7,700 行，那份是導覽圖。
...
| 素材 | 結論 |
|---|---|
| CGA `.PIC` / `.SHP` | **已驗證**。`OPEN.PIC` 是 320×200 2bpp **linear**（不是硬體 odd/even 交錯）；人像框 144×144；sprite frame **16×32** |
| EGA `.PIE` 全螢幕圖／人像框 | **已驗證**。16 bytes 內嵌調色盤 + 4-plane sequential、MSB-first，高度 ×1.75（144→252） |
| EGA `.SHE` 精靈圖 | **未解**。frame 邊界 448 B 確定，但 plane 內部佈局試過八種假設全是雜訊 |
| `OPEN.PIE`（102,160 B）| **未解**。確認不符 3.5 倍規律 |

過程中踩到兩個「算術對但方向錯」的陷阱：sprite 是 16×32 不是 32×16（byte 數兩者相同），
`OPEN.PIC` 是 linear 不是交錯。兩者都只有肉眼比對抓得到——
這也是本專案把「視覺產物一律 dump 出來比對原版」列為硬性驗收條件的原因。
