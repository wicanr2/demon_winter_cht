...
| 畫面圖 EGA | `*.PIE` ×10 | 5,184 → 18,160 = **3.5× + 16 B**。`OPEN.PIE` 是已解的 608×336 例外：16 B 調色盤後為逐列四 plane，見 `docs/formats/graphics.md` §5 |
...
- **不成立的部分**：`OPEN.PIE` 不套用 3.5 倍規律；它已另解為 608×336、
  16 B 調色盤後逐列四 plane 的開場圖，見 `docs/formats/graphics.md` §5。
...
| ~~V2~~ | ~~EGA 素材是否真為 320×350 4-plane?~~ | **已解 2026-07-25**：`.PIE` 人像框、`.SHE` 精靈、CGA `.PIC/.SHP` 與 EGA 640×350 畫面均已驗證；`OPEN.PIE` 的例外另由 V3 收尾。見 `docs/formats/graphics.md` |
| ~~V3~~ | ~~`OPEN.PIE` 102,160 B 為何不符 3.5× 比例?~~ | **已解 2026-07-26**：它是 608×336、非半寬的開場圖；16 B 調色盤後為 336 列 × 每列 4 plane × 76 B。以列間差異窮舉找到 304 B stride，並與 CGA 同圖肉眼對照。見 `docs/formats/graphics.md` §5 |
...
- [x] `PARTY.DAT` 存檔格式 → 可讀可寫；姓名、種族、屬性、道具欄、座標、
      朝向與時間已解。職業在 `+0xf6`，已學技能為 `+0xc8`–`+0xe6`
      的 31-byte 旗標陣列，載入／寫回及逐欄測試已完成；見
      `docs/formats/game-data-tables.md` §1.6、§4。
...
- [x] remake 玩家文案已資料分離：原版資料字串 500/500，另有 839 條
