...
推測 EGA 版素材為 **320×350、4 bitplane**(EGA 640×350 模式下水平 pixel doubling)。
`.PIE` 多出的 16 bytes 推測是 16 色調色盤(EGA 16 色暫存器值)。此假設必須在階段 2 用實際解碼驗證。
...
| V5 | `DATA*.TXT` 的數字欄位語意(事件 opcode?條件?跳轉?) | 反組譯讀取這些檔的函式，對照遊戲內實際觸發 |
...
| 風險 | 影響 | 對策 |
|---|---|---|
| `DATA*.TXT` 事件表語意解不出 | 劇情流程無法重建，遊戲「能跑但玩不通」 | 反組譯讀取函式 + DOSBox 逐事件觸發對照;必要時用截圖 oracle 反推 |
| 反組譯進不了主邏輯(indirect call / jump table) | 演算法只能靠黑箱觀察 | 用「線索常數」跳進去(字串位址、檔名、magic 值);配合 DOSBox 動態追蹤 |
| 8087 浮點運算的精度差異 | 戰鬥結果與原版對不上 | 對拍測試找出差異點;Go 用 float64 對齊 x87 double |
| 中文塞不進原版版面 | UI 破框 | 640×400 畫布已預留空間;仍需逐畫面截圖驗收 |
| 原版無配樂(僅 speaker 音效) | 「音樂還原」目標需重新定義 | V1 先驗證再決定範圍;若確實無配樂，如實記錄，不自製配樂冒充原版 |
