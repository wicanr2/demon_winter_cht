...
- **成立的部分**:`.PIE` 全螢幕圖/人像框確為 4-plane sequential + MSB-first,
  高度確實 ×1.75(人像 144→252);多出的 16 bytes 確實是內嵌調色盤(用標準 EGA 調色盤
  會失真,反證內嵌表為真)。
- **不成立的部分**:`.SHE` 精靈圖**不適用**同一佈局(八種假設全是雜訊,仍未解);
  `OPEN.PIE` 不符 3.5 倍規律。
- **「320×350」這個具體尺寸並未被證實** —— 已驗證的是人像框 144×252 這類個別尺寸,
  不是全域畫布尺寸。

> **教訓**:檔案大小的算術關係是很好的**起點**,但它只約束「總位元數」,
> 不約束佈局方向。同一組 byte 數可以對應 16×32 或 32×16、linear 或 interleaved、
> row-major 或 column-major —— 這幾種本專案**全都實際踩過**。
> 算術自洽只是必要條件,肉眼比對原版才是充分條件。
...
| ~~V2~~ | ~~EGA 素材是否真為 320×350 4-plane?~~ | **部分已解 2026-07-25**:全螢幕圖/人像框 `.PIE` 已驗證(16-byte 內嵌調色盤 + 4-plane sequential,高度 ×1.75);CGA `.PIC`/`.SHP` 全解。**`.SHE` 精靈圖的 plane 佈局未解**(frame 邊界 448B 確定,內容仍雜訊)。見 `docs/formats/graphics.md` |
...
- [ ] 頂層狀態機的 overworld 移動 / town 進出迴圈(卡在 decompiler 品質,需逐行讀 asm)
- [ ] 時間欄位語意(進位機制已解,但 wrap 值 38/35/23 對不上「一天 26 小時」)
- [ ] 法術效果、7 個戰鬥選單動作(Attack 以外)的細節
...
- [ ] `.FNE` / `.FNT` 字型 → PNG 字表(**解碼仍是雜訊**;中文化的前提)
...
- [ ] `assets` 套件:所有格式的 Go 解碼器
- [ ] 渲染層:640×400 畫布，EGA 16 色調色盤，sprite blit
- [ ] 世界地圖移動、城鎮進出、地城探索
- [ ] 戰鬥系統(回合、移動點數、命中/傷害、法術)
- [ ] 角色系統(建角、屬性、技能、裝備、升級)
- [ ] 事件腳本直譯器 + 對話/劇情顯示
- [ ] 存檔 / 讀檔(與原版 `PARTY.DAT` 雙向相容)
- [ ] 音效還原(依 V1)
...
- [ ] 從 `*.TXT` 抽出劇情文本 → 翻譯表
- [ ] 16×16 CJK 點陣字型(含 `GOT.FNE` 花體風格的中文標題處理)
- [ ] 統一譯名表(角色/地名/法術/道具/怪物)，防止跨檔漂移
- [ ] UI 版面重排以容納中文
- [ ] 遊戲內標題畫面中文化
