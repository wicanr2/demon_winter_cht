...
#   rep 5 Up            同一個鍵送 5 次（每一步後自動關掉跳出來的文字框）
#   wait 1.5            等幾秒（讓動畫／載入跑完）
#   shot 名稱           截一張圖到 <輸出目錄>/名稱.png
#   #  註解
#
# **`rep` 會自動關文字框**，因為不關就走不下去 —— 這是第一次跑
# 全程試玩才發現的：踩到事件格會彈出文字框，它把後面所有方向鍵
# 全部吃掉。腳本照樣一路送鍵，軌跡卻停在那一格不動，
# 看起來像「走到一半卡住」，其實是模態畫面擋著。
# **單點截圖驗收永遠遇不到這件事**，因為它只按幾下就拍照。
#
...
# 關掉擋路的文字框。上限 5 次 —— 關不掉就是真的卡住了，
# 讓它留在軌跡裡，不要用無窮迴圈把問題藏起來。
clearbox() {
...
while read -r cmd arg rest; do
    case "$cmd" in
        ""|"#") ;;
        key)  send "$arg" ;;
        rep)  for i in $(seq 1 "$arg"); do send "$rest"; clearbox; done ;;
        wait) sleep "$arg" ;;
