...
//
// `unit+0x20` 是**陣營**，值域 {1, 2, 4} 對 {11, 12, 14}，`>= 10` 是玩家側。
// 這個欄位一度被判為未解並公開收回過，這一輪已經釘死 —— 六處證據與魅惑的
...
//
// 原版寫死成「目標 `+0x20` > 10」（0x79ef）。這裡寫成「兩邊陣營不同」，
// 對怪物側的施法者完全等價，而且被魅惑的玩家角色（陣營 2）也會照樣挑
// 玩家當目標 —— 那正是原版的行為。
func (b *Battle) validAITarget(of, t *Unit) bool {
	return t != nil && t.Alive() && t.OnPlayerSide() != of.OnPlayerSide()
}
