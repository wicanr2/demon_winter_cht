...
// （internal/assets/world -> 往上 3 層就是 repo 根目錄）。
func repoRoot(t *testing.T) string {
	t.Helper()
	_, file, _, ok := runtime.Caller(0)
	if !ok {
		t.Fatal("world: runtime.Caller 取得目前檔案路徑失敗")
	}
	// file 是 .../internal/assets/world/testdata_test.go
...
func origDataDir(t *testing.T) string {
	...
	if _, err := os.Stat(dir); err != nil {
		t.Skipf("world: 找不到原始遊戲資料目錄 %s，略過需要真實檔案的測試: %v", dir, err)
	}
	return dir
}
