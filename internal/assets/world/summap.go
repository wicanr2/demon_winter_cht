...
//
//  4. **tile 0x14 的條件替換不在這裡做。** 原函式在寫入前會檢查
//     `tile == 0x14`，成立就呼叫 15be:179f，回傳 1 時改寫成 0x62
//     （0x1b01e／0x1b075 兩個分支都有）。那個判斷式已經追出來了：是個
//     擲硬幣的亂數，不是季節旗標——0x14 與 0x62 是兩種海面浪花圖塊，
//     原版在載入時隨機摻著鋪，讓大片海洋不要一模一樣。
//     見 ocean.go；本函式維持輸出檔案解出來的原樣。
//
