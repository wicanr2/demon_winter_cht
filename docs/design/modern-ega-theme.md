# Modern EGA 主題：美術與整合規格

> 狀態：設計定案稿（2026-07-29）
>
> 目標：保留《冬之魔》EGA 版的構圖、地形辨識與 SSI 時代氣質，重畫成較
> 細緻、較易讀的現代像素素材；原版 EGA／CGA 永久保留，以 `F8` 依序輪替。
...
- 清楚的像素邊緣。不得使用照片材質、向量漸層、平滑字型或模糊濾鏡。
- 倚天 16×15 中文字型與目前 640×400 邏輯版面。
