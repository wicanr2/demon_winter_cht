...
> 狀態：世界、單位與地城 59／59 覆蓋完成；待 P4 使用者最終畫面審查
>（2026-07-30）
>
...
### 2026-07-30 客觀覆蓋與地城邊界

- 世界正常／冬季實際索引差集皆為零。
- 怪物 runtime 224/224 幀具獨立 A/B 相位。
- 隊員 runtime 24/24、海戰 runtime 24/24。
- 世界 `tiles.normal`／`tiles.winter` 與地城 `dungeonTiles` 已在 JSON
  manifest 分成獨立 namespace；相同 index 不會跨語意取圖。
- `dungeonTiles` 目前為空，地城安全保留歷史主題底稿。這是誠實的漸進載入，
  不是完整地城重畫。

第一張地城材質方向稿：

![Modern Icon 地城方向稿](img/modern-icon-dungeon-direction-v1.png)
...
Modern Icon 不再塞入舊 `32×28` atlas。`-modern-icon-dir` 載入逐索引列舉的
`64×56` 不透明 PNG，並在 1280×800 最終畫布直接覆寫地圖格；規則、索引與
碰撞仍走原版資料。世界 normal／winter 與地城 `dungeonTiles` 分開查表；
未列出的索引保持相容底稿，避免量產未完成時以假圖補齊。

首輪森林母稿已實際進入這條路徑，證明座標與載入契約可行；但單一紋理重複
造成接縫，只通過架構、不通過美術。完整證據與裁決見
[`docs/playtest/20`](../playtest/20-modern-icon-high-resolution-layer.md)。
...
- 工作檔可以 64×56 繪製，但輸出前必須由美術師手工修成 32×28；禁止直接用
  雙線性縮圖當成完稿。
- 地形與戰鬥 sprite 的原始資料是「整格覆寫」；探索步行人物已依使用者
  裁決改為腳下地形＋透明遮罩（`docs/playtest/56`）。Modern Icon 採獨立
  高解析透明人物。
