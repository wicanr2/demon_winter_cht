...
第三次加入本場景唯一的 `0x17` 左海右陸岸線。它依原索引輪廓獨立重畫，沒有把
concept 海岸直接縮成一格：

![Modern Icon 0x17 海岸實機試片](../design/img/modern-icon-m1-coast-runtime.png)

`0x17` 格內的海、沙、草過渡清楚；但上下相鄰的其他岸線索引尚未重畫，仍可看到
新舊素材邊界。這證明岸線必須按所有方向／轉角成組交付，單格通過不等於海岸完成。

## 3.2 冬季一一對應

`0x14/17/23` 已各自從常態母稿製作冬季對應，保持索引、岸線方向與海陸比例。
同一固定場景按 `Tab` 切換冬季 terrain：

![Modern Icon 冬季 M1 實機試片](../design/img/modern-icon-m1-winter-runtime.png)

- 61 格雪原維持低頻、可讀，沒有規律亮點；
- 冬季深海仍明確可航行，沒有被畫成完整冰層；
- `0x17` 內海／岸／雪地關係正確；
- 其他岸線、樹木、隊伍與特殊格仍是 WINTER.SHE 相容底稿，新舊邊界清楚可見。

因此常態／冬季三索引配對通過本場景，但完整冬季 atlas 尚未完成。
