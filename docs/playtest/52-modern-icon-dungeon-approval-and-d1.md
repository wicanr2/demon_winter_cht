...
> **2026-07-30 後續取代：**使用者再次以原版參考指出黑框是差異，要求
> EGA／CGA 步行人物透明。remake 現已先畫腳下地形再疊遮罩人物；本節以下
> 只保存當時誤判的歷史理由，不再是產品裁決。見 `docs/playtest/56`。
...
當時因此決定 EGA／CGA 不改透明；此決定現已撤回，由 `docs/playtest/56`
的使用者裁決取代。
