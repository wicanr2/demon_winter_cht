...
| `trailer` | **未解** | 長度 0–3 個數字，內容多為 `1, 255` 或單獨 `255`，少數是具體小數字（`1`/`2`/`3`）。推測可能是「強制移動出口 / 事件已觸發旗標 / 下一段劇情索引」之一，但無足夠證據確定，見 §4 |
...
| `%` | 8 | 內容是**符文/密語提示**，例如 `%YMROS.IS...MINE`、`%RING.BELL...AT.....MIDNIGHT`、`%DIVINITY`，句中空格被替換成句點 `.` | **已驗證內容語意**（對得上攻略的密語謎題），**推測**符號用途：`%` 可能標記「這段文字要用特殊字型/等寬符文顯示」，句點取代空白可能是因為符文顯示程式用固定格數排版 |
...
5. **`T2C.TXT`、`T2D.TXT` 的觸發條件與 `T.TXT`/`EREGORE.TXT`/`WIN.TXT` 的觸發時機。** 這些純文字檔沒有
