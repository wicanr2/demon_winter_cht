...
| `%` | 8 | 內容是**符文/密語提示**，例如 `%YMROS.IS...MINE`、`%RING.BELL...AT.....MIDNIGHT`、`%DIVINITY`，句中空格被替換成句點 `.` | **已驗證**（`docs/re/02` §2.4，`docs/re/72` 補上字型檔）：`%` 標記「用符文字型顯示」。`FUN_25be_18fa` 從 `param_1+1` 起逐字元轉換（`'.' → 0`、其他 `→ char − 0x40`），以 9 欄一列的網格畫出 —— 所以 `'.'` 是**空白 glyph 的專用替代字元**，不是排版巧合。`0x1a85c` 判斷字串首字元 `0x25`（＝`%`）才呼叫 `25be:18fa`，而那支的第一件事就是載入 `CYPHER.SHP`（1728 bytes ÷ 64 ＝ **27 個 16×16 符文字形**，26 字母 + 句點）。句點取代空白是因為**字型裡有句點的字形、沒有空白的字形**。原本標「推測」的兩點都成立 |
