...
每座城鎮的 `(X, Y)` 在 `DEMON.INT` 的 `DS:0x2f86`（檔位移 `0x28a86`），25 組 byte 對；
踩到 tile `0x2e` 或 `0x64` 才會去查這張表。座標全表、反組譯出處與 25/25 的
交叉驗證見 `docs/spec/08-town-economy.md`「進城」一節。
...
> **本節原本的猜測表已作廢。** 那張表是在「找不到 tileset 圖檔、只能靠出現頻率猜」
> 的前提下寫的，而 `DEMON.SHP`／`WINTER.SHP` 後來解出來了（102 個 16×16 圖塊，
> 見 `docs/formats/graphics.md`），可通行性表也解出來了（`FILES.DAT`）。
> 舊表至少錯一條：tile 98（`0x62`）被猜成「牆」，實際上是**海面**。
> 頻率猜測在地城裡看起來合理，是因為地城本來就沒有海。

已有證據的：

| tile | 語意 | 依據 |
|---|---|---|
| `0x14`、`0x62` | **海面**（同一種地形的兩種浪花，載入時隨機摻著鋪）| 反組譯 `15be:179f`／`15be:17c0`；兩者可通行性皆 `0xff`；圖塊都是橫向浪紋，`0x14` 多了白色浪沫。見 `docs/re/03` §3.4 |
| `0x2e`、`0x64` | **城鎮入口** | 踩到才會查城鎮座標表（`0x19192`）；全世界 27 格，25 格對得上 25 座城鎮。見 `docs/spec/08-town-economy.md` |
| `0x0e`、`0x2b` | **山丘**（移動多算一步）| 可通行性表裡唯二的 `0x03`；圖塊是起伏等高線；與手冊「山丘通過時間為一般地形兩倍」吻合 |
| `0x0f`、`0x10` | 可通行性 `0x02` 的唯二兩格 | 分組證據，地形種類未定 |

### 地形類別＝可通行性值（已解，`docs/re/24`）

可通行性表把 102 個 tile 分成 11 組。**走得到的那八組（值 0–7）就是地形類別**
—— 引擎沒有第二張地形表，同一張表移動時當「能不能走」、遭遇時當「這是哪一種
地形」（`FUN_222f_2a5b` 的第三行）。

| 值 | 地形 | tile |
|---|---|---|
| 0 | 森林 | `0x01`–`0x0c` |
| 1 | 平原 | `0x17` `0x1a` `0x1d` `0x20` `0x23` `0x27` `0x29` `0x38` `0x4f` |
| 2 | 沼澤 | `0x0f` `0x10` |
| 3 | 山丘 | `0x0e` `0x2b` |
| 4 | 地城地板 | `0x00` `0x13` `0x53` |
| 5 | 凍土 | `0x5a` |
| 6 | 地城地板（另一種）| `0x56` |
| 7 | 沙漠 | `0x28` `0x43`–`0x46` |

手冊的「Kudzu 樹叢」沒有自己的類別（八組裡六組對上手冊、兩組是地城地板）。
判定依據與證據等級見 `docs/re/24` §4 —— 只有山丘是直接驗證，其餘是推定。

> ⚠ 本節原本寫「**組別本身不等於地形種類**，分組只能縮小範圍」。**那句話講反了**，
> 已刪除。當時是看到 `0xfe` 一組有 19 格、城鎮 tile 混在裡面就否決了 ——
> 但 `0xfe` 屬於不可通行那一側，本來就不參與地形分類。
...
### 2.4 SUM.MAP：已解 — 23 張 64×64 子地圖的 RLE 串接

15743 bytes = 23 個變長 RLE 區塊首尾相接，每個解出一張完整的 64×64 子地圖。
區塊的 ID 表與長度表都在 `DEMON.INT` 裡（`31f0:2488` / `31f0:24b6`），
`SUM.MAP` 本身不含任何自我描述的索引。演算法、兩個 bug 的修正過程與驗證
見 `docs/re/03-audio-and-resources.md` §3.2/§3.3；實作見
`internal/assets/world/summap.go`。

> **本節原本寫「未解，判斷不是簡單壓縮也不是簡單方陣」，已推翻。**
> 當時的推論鏈整條都是對的方法用在錯的假設上：
>
> - 「15743 不是任何合理正方形」→ 對，但它本來就不是一張圖，是 23 張串接。
> - 「熵 5.53 bits/byte，不像典型壓縮」→ 對，RLE 的熵本來就介於原始資料與
>   通用壓縮之間，這個數字其實正好符合 RLE，當時卻拿它當「不是壓縮」的證據。
> - 「測過 RLE 假設，解出 156865 個 tile，予以否證」→ **值與次數的欄位順序猜反了**。
>   實際是 `byte & 0x7f` 是值、下一 byte 是次數。一次否證性測試失敗就把整個
>   方向劃掉，是這裡最貴的一步。
>
> 教訓：否證一個「假設 + 一種實作方式」，不等於否證那個假設。真正解開它的
> 是走 `rulebook/62` 去找讀取函式，而不是繼續在資料上做統計。

### 2.5 世界是 7×7 格的子地圖網格（已驗證）

**子地圖編號 = 欄×10 + 列**，欄是十位數（世界的 X 軸）、列是個位數（Y 軸），
兩者都從 1 數到 7 —— 合法的世界編號是 11–77。**編號小於 10 的不是世界地圖，
是地城／室內**（`MAP1/3/5.MAP` 與 `SUM.MAP` 裡的 2、4，共五座）。

> 本節原本標「假說，證據中等」，靠的是攻略的方位敘述與接邊 tile 一致率
> （72.2% vs 隨機基線 43.9%）。那些都只是旁證。現在有直接證據了，
> 而且順便修正了假說裡的一個錯：舊表把 `MAP1/3/5` 畫成「欄 0」的一行，
> 其實它們根本不在世界網格上，是地城。

證據是走到子地圖邊界時的換圖程式碼（`DEMON.INT` 0x16fec–0x17114）。
四個方向各一段，形狀完全對稱：

| 方向 | 觸發條件 | 擋住的條件 | 否則 |
|---|---|---|---|
| 往西 | `X == 3` | 欄 == 1 | `map_id -= 10`、`X ← 59` |
| 往東 | `X == 60` | 欄 == 7 | `map_id += 10`、`X ← 4` |
| 往北 | `Y == 3` | 列 == 1 | `map_id -= 1`、`Y ← 59` |
| 往南 | `Y == 60` | 列 == 7 | `map_id += 1`、`Y ← 4` |

「X 方向動十位數、Y 方向動個位數」把兩個軸釘死；擋住的條件同時給出
網格範圍。拆欄／列的指令是 `idiv 10`：商是欄（`0x16ffd`、`0x170e2`），
餘數是列（`0x1704c`、`0x17097`）。

順帶確認兩件事：

- **隊伍欄位 `+0xa3` 是目前的子地圖編號**。上面四段都在改它，而別處拿
  `>= 10` 當「在戶外還是在地城」的判斷（戰場視野要不要查時辰表，
  見 `docs/spec/02-combat.md`）—— 剛好對上「10 以下是地城」。
- **一張子地圖走得到的範圍是 4–59**，不是 0–63。四周各三格是換圖用的邊界，
  踩上去就被送到隔壁那張圖的另一側。

實際存在的世界子地圖有 21 張（`SUM.MAP`），7×7 = 49 格裡其餘的推測是純海洋，
沒有存檔。實作與測試見 `internal/assets/world/grid.go`。
