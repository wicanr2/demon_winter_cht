...
可通行性表把 102 個 tile 分成 11 組（`0xff` 39 格不可步行、`0xfe` 19 格、`0x00` 12 格、
`0x01` 9 格、`0xfd` 9 格、`0x07` 5 格、`0x04` 3 格、`0x02`／`0x03` 各 2 格、
`0x06`／`0x05` 各 1 格）。組別本身不等於地形種類 —— `0xfe` 一組就有 19 格，
城鎮的 `0x2e`／`0x64` 只是其中兩個。要把手冊的七大地形對到 tile 值，
分組只能縮小範圍，還需要逐格看圖。
