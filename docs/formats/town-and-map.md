...
### 2.5 世界是 26 張子地圖組成的網格（假說，證據中等）

26 張子地圖 = `SUM.MAP` 的 23 張 + `MAP1/3/5.MAP`。ID 若讀成
**`欄 × 10 + 列`**，剛好排成一張沒有重號的網格：
...
1. **攻略的方位敘述**（三條，全對）。把子地圖座標換算成世界座標
   （`欄×64 + X`, `列×64 + Y`）之後：Idlewood（35）在起始大陸（34）**正南**，
   攻略寫「it's to the south of the continent you start on」；Asaht（43）
   在 New Gleon（44）**東北**，攻略寫「from New Gleon, head northeast to a
   landmass with the town of Asaht」；Terlabba（52）在 Asaht **東北**，
   攻略寫「From there, sail northeast to … Terlabba」。
2. **接邊連續性**：相鄰子地圖「右緣 vs 左緣」的 tile 一致率 72.2%，
   隨機配對的基線是 43.9%。
3. 反過來讀成 `列 × 10 + 欄` 的話，三條方位敘述全錯，接邊一致率也只有 44.9%
   （與基線無異）。

不標「已驗證」的理由：72% 對 44% 是訊號但不是鐵證（海洋 tile 佔比高，
基線本來就不低），而攻略方位是人寫的散文，不是原始資料。要坐實它，
最直接的作法是在 DOSBox 走過子地圖邊界、看座標怎麼跳。

`ITEMLOCB.DAT` 的 `map_id` 只出現 1/3/4/5（見 §4.2），落在欄 0 那一行，
與這張表不衝突。
