...
void __cdecl16far FUN_25be_17fe(int param_1)   // param_1 = 這個區塊的 byte 長度
{
  ...
  do {
    ...
    if ((char)bVar2 < 0) {          // 最高位=1：RLE run
      local_4 += 2;
      count = 來源[local_4-1];      // 下一個 byte 是「重複次數」，0 代表 256
      do {                            // do-while：先寫再減，所以 0 會繞滿 256 圈
...
> **⚠ 2026-07-26 修正（又抓到兩個，都是「解得出東西但是錯的」）**
>
> **(1) 重複次數 0 代表 256。** 上面的虛擬碼是 `do { … } while (count != 0)` ——
> **先寫一格再減**。次數 byte 是 0 時，`dec` 之後變 `0xff`，要繞完整整 256 圈才回到 0。
> 對應的機器碼：
>
> ```asm
> 1b01b  loop: inc  word [bp-0x4]        ; 已寫格數++
> 1b048        cmp  word [bp-0x4],0x1000 ; 寫滿 4096 就收工
> 1b04d        jne  1b057
...
> ```
>
> 8 位元計數器要表達「256 連」只有這一種寫法，是 RLE 的常見手法。照字面當 0 圈解，
> 每碰到一次就少 256 格。
>
> **(2) 每個區塊的第一個 byte 不是 RLE 資料。** 原程式讀檔時把區塊寫進
> `[ds:0x4c90]+1`（`0x188b5` 的 `add ax,0x1`）。跳過這個 byte 之後 23/23 個區塊
> 精確填滿 4096 格；不跳過則全部卡在 `4097 - 256k`。這個 byte 語意未解，
> 值域 `0x01`–`0x09`（區段 2 與 4 是 `0xa3`），疑似與獨立 `.MAP` 檔開頭那 1 byte
> header 是同一種東西。
>
> **這兩個 bug 為什麼活了這麼久**：症狀是「後續 tile 整批往前位移」，畫出來還是有
> 海岸線有陸塊，格數／值域／size 加總全部照樣通過。抓到它的不是測試，是**城鎮座標表**
> —— `DEMON.INT` 裡有一張 25 筆 `(X, Y)` 的城鎮座標常數表（見
> `docs/spec/08-town-economy.md`「進城」一節），與 `SUM.MAP` 完全無關。修正前 25 個
> 座標上一個城鎮 tile 都沒有，修正後 25/25 全中。
>
> **教訓**：當時把觀察到的錯誤結果寫成了規格 —— 文件原本寫「讀完 bytes 就停，
> 不一定填滿 4096」，測試也照著寫成「不斷言等於 4096」，等於用測試把 bug 釘死。
> 現在 `TestLoadSumMap_AllSegmentsFill` 嚴格要求 23/23 都填滿。
>
### 3.4 tile `0x14` → `0x62`：海面浪花的隨機摻色（已解）

解壓時（以及其他幾條地圖載入路徑上）會檢查 `tile == 0x14`，成立就呼叫
`15be:179f`，回傳 1 才改寫成 `0x62`。
...
一個乘法同餘產生器，種子在 `DS:0x2f4c`（檔位移 `0x28a4c`）初值 `0x2711` = 10001。
與遊戲規則用的那個浮點 LCG（`docs/spec/01-rng.md`）不是同一個。

**這兩個 tile 是什麼**：都是橫向浪紋的海面，差別只在 `0x14` 摻了白色浪沫。
`FILES.DAT` 可通行性表兩者都是 `0xff`（不可步行），所以替換**純屬外觀** ——
原版用它讓大片海洋不要一模一樣。

佐證來自另外兩個呼叫點（`0x49f4`、`0x15fff`）：兩處都是先
`setmem(地圖緩衝區, 4096, 0x14)`（整張圖填成海）再呼叫這個替換函式
（`15be:17c0` = 檔位移 `0x1afa0`，線性掃過 4096 格做同樣的事）。
「填滿海 → 摻浪花」這個組合只有在 `0x14`/`0x62` 是同一種地形的兩個變體時才說得通。

實作見 `internal/assets/world/ocean.go`。因為是純外觀，本專案把它放在算繪端、
不寫進 `Map` 資料裡，`TestOceanDither_PurelyCosmetic` 守住「兩者可通行性相同」
這個前提。
...
一致**，這代表 `SUM.MAP` 的 23 個區塊，語意上就是 23 張壓縮過的 64×64 地圖。

~~（可能不滿版，因為讀完 `param_1` bytes 就會提早停止，不一定填滿全部 4096 格）~~
—— **已推翻**：那是上面 2026-07-26 修正的兩個 bug 造成的假象，不是格式特性。
23/23 個區塊都精確填滿 4096 格。

**已完成的收尾驗證**：`internal/assets/world/TestDumpWorldMaps` 會把 23 張子地圖
加上 `MAP1/3/5` 共 26 張全部算繪成 PNG（城鎮格標紅框）輸出到 `workplace/dump/maps/`，
已肉眼比對過 —— 修正後是連貫的大陸、海岸線、森林與山脈，修正前是雜訊。
數值面的收尾驗證見 `TestTownSites_LandOnTownTiles`（25/25 城鎮座標對上地圖）。
