...
| `PARTY.DAT` 角色記錄 `+0xe6` | 未在既有文件出現 | ~~**[假設]**:盾牌裝備旗標,非零時護甲值 `+2`~~ → **已推翻**:那是技能旗標陣列的第 30 格(`0xc8 + 30`),即**硬化皮膚**;這遊戲的 `ITEMS.DAT` 沒有盾牌。見 `docs/re/103` §2 |
...
| 護甲最終數值來源 | 未提及 | **已驗證**:`0x4ebe` = `表[型別 − 7] + 附魔`,表在 `31f0:16c5`,再視**硬化皮膚技能**(不是盾牌旗標)`+2`。~~表格內容本次未逐一核對~~ → 內容 `00 01 02 04 05 06 03`,**不是等差**,見 `docs/re/103` §4。⚠ 查表的鍵是**道具型別**不是「裝備槽 byte0」 |
...
4. **`field_4ed6`、`field_4ed2`、`0x64e` 旗標表的確切語意**:命中/傷害公式裡多處
   ...
   遊戲規則(職業技能?種族被動?)。
5. ~~**裝備護甲最終數值表**(`31f0:16c5`)~~ **已核對完畢**,見 `docs/re/103` §4;
   同一輪還解出旁邊的**職業護甲上限表**(`31f0:03de`,§5)。
   武器附魔加成的另一張表(`+0x15`/`+0x17` 偏移那段)仍未逐位元核對。
