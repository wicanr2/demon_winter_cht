...
- 每個單位一回合可以:移動(尋路,`FUN_222f_1404` / `FUN_1990_050d` 找最近目標)、
  攻擊(選單 case 0xa,呼叫 `FUN_1990_0002` → `FUN_138d_25da`)、其他選單動作
...
| case | 已知行為 | 信心 |
|---|---|---|
| 1 | 複雜的傷害/移動點數處理,結尾呼叫 `FUN_138d_1e19`(未直接接到 §2/§3 的核心命中函式) | 假設:可能是「Cast」或帶特殊動畫的攻擊變體,未定案 |
| 0xa | 呼叫 `FUN_1990_0002`(尋路+呼叫 `FUN_138d_25da`) | **已驗證**:這條路徑確實走到本文 §2/§3 完整分析過的命中/傷害引擎,是「一般攻擊(Attack)」在選單上的最可能對應 |
| 0xd | `Move pts` 相關計數器 -2、鏡頭歸位、回傳特殊碼 `0x20` | 假設:疑似「撤退/逃跑」相關動作 |
| 3 | 一大段涉及多個角色屬性表(`0x530c` 一帶陣列)的複雜運算,疑似法術/範圍效果 | 假設:「Cast」或群體效果,未深入(超出本輪範圍) |
| 4 | 涉及道具清單、文字排序/選擇邏輯 | 假設:「Use」道具,未深入 |
| 0xc | HP 上限鉗制與回復 | 假設:疑似「Leech」或休息類回復動作 |
| 其他(0,2,5,6,7,8,9,0xb,0xe) | 有反編譯內容但未逐一比對到具體選單字串 | 未定案 |
...
`FUN_2cdc_033d` 的選單字串來源表)。除了 case `0xa`(已確認=Attack 路徑)之外,
其餘 7 個動作的精確 case 編號**留給下一輪**(任務單本身也允許不用全解法術/
特殊動作的細節)。
...
3. **完整的選單 case ↔ 動作名稱對照表**:目前只確認 `0xa`=普通攻擊,其餘
   7 個命名動作(Cast/Use/Turn Undead/Dodge/Examine/Pray/Leech)的 case 編號
   未定案。
