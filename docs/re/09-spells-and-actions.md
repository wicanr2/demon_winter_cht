...
print caster_name;                       // 指標表 0x4c98/0x4c9a
roll = RNG(100);
...
不會隨戰鬥結束重置——**[假設]**：這代表遞減是永久性的，除非有其他地方會重置這個值,
本次未追出初始值設定點或重置點，只確認遞減量。任務單提到「手冊說 20% 成功率」，
若初始值確為 20，配合 -5 遞減，前幾次成功率序列會是 20%→15%→10%→5%→0%——這與
「每次成功後遞減」的描述完全吻合，但**初始值 20 本身未在本輪反組譯中直接核對到**
（[假設]，需要下一輪找出 `+0xeb` 的初始化點，很可能在角色建立或每次戰�vo開始時）。
...
- 幻術每回合消失的機率：**[未解]**，`FUN_138d_065e`（法術/AI 選擇引擎，§2.5 提過）
  內有一段 `local_c = *(byte*)(local_c + 0x1680) + RNG(...) - 1` 之類的邏輯疑似與
  「每回合重新判定是否維持召喚」有關，但沒有足夠時間單獨驗證，[未解]。
- 「建立當回合不能行動」的實作方式：**未直接驗證**，但與 §1.1 主迴圈已知的行動佇列
  機制（`docs/re/06` §1.1：每回合重建佇列，`if unit.status<2 && exists && status_counter>=0`
  才會排進佇列）邏輯上相容——若新召喚單位的某個初始狀態欄位使它在**建立當下**
  被排除在本回合佇列之外，下回合重建佇列時才會被納入，這是最合理的實作方式，
  但沒有具體位元組層級的證據支持，[假設，未解]。
