...
- **城鎮經濟（商店定價／說服／治療所／客棧）：反組譯層級未解。**
  相關函式（`FUN_278d_0932` 神殿、`FUN_2aed_14c2` 角色/技能畫面等）decompiler
  嚴重損毀，且對應字串（`Healers`／`Haggle`／`Persuasiveness` 等）**完全沒有被
  Ghidra 自動分析找到任何引用指令**——跟建角 UI 是同一類「未被發現的程式碼區」，
  不是隨便撞牆就放棄，是逐一排查過call site 之後的誠實結論。已列出具體位址與
  下一步（§3、§8）。
- **神殿祈禱／神祇機制：同上，未解。**（§4）
