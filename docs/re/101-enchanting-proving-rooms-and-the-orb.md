# 101 — A7 最後四格：附魔工坊、十間試煉室、惡魔水晶與恆世寶珠
...
| 8 | 地圖 2 (42,28) | **恆世寶珠**（道具 29）—— 要十間試煉室全過 | **`docs/re/80` §3 那個「找不到的 `+0xb9 = 1` 寫入端」就是這裡** |
...
而 `ITEMS.DAT` 第 29 件是 **`Orb/Evertime`**（恆世寶珠）。
...
| 6 | **case 8**（地圖 2 (42,28)）| **恆世寶珠** |

---

## 5. 接上引擎之後（case 7／8／9 已實裝）

### 5.1 攻略把整套機制講了一遍，逐句對上

`docs/walkthrough/part-4.md` §3.5（白騎士地窖，THE VAULT OF THE WHITE KNIGHTS
＝ 地圖 2）：

> 這裡有**十個試煉室，每個職業各一間**，你需要全部走過一遍。
> **如果隊上有對應職業的角色，就得跟特定敵人單挑一對一決鬥**，
> 唯獨**盜賊那間是陷阱，要偵測並解除**。據說**學者那間只會拿到多一點線索**。
> 全部完成後，回到火焰廳的小壁龕，就能領取寶珠（恆世寶珠）。
> **離開後第一次睡眠時，你會收到馬利馮的警告。**

十間、一職業一間、單挑、盜賊是陷阱、學者只給線索、全過才給寶珠、
拿了寶珠之後睡覺播預言 —— **七項全中**。

盜賊那間的「陷阱」也在資料裡：`2SS.DAT` 有一筆 **(42,14) 類別 3**，
就在盜賊試煉室 (42,13) 的正南方一格。所以類別 5 那一格只負責印那句嘲諷，
真正的考驗是隔壁那個陷阱 —— 而陷阱那條路 A1 早就接好了。

### 5.2 接上去才發現的兩個既存缺陷

**① 佈陣的「保險」把單挑機制整個廢掉。**
`startBattle` 原本寫著「陣型裡沒有他 —— 原版佈陣是掃九格，沒被放進去的人
不會上場。**本專案不讓人憑空消失**：塞進中心附近還空著的位置」。
那層保險在不知道有機制依賴它的時候加的 —— 試煉室的單挑正是靠
「清空陣型、只填回符合職業的人」做出來的，保險一開就五個人全上場。
實跑抓到（遊俠的試煉室畫面出現五個「我」）之後改回原版：不在陣型裡就不上場。

> **教訓：「本專案不讓 X 發生」這種保險要寫下它擋掉了什麼可能性。**
> 這一條擋掉的正好是原版用來做單挑的那個手法。

**② `Consume` 之後那一格會掉到 `DATA*.TXT` 路徑，顯示無關的房間敘述。**
試煉室過關會把 `nSS` 記錄的屬性清 0（原版 `0x0f6a3`／`0x0e1f4`）。
清 0 之後類別是 0，而引擎的 `TriggerLookup` 沒有擋類別 0 ——
於是 `hit.EventIndex` 變成 0，畫面跳出第 0 筆房間文字。
實跑抓到的症狀是走回過關的盜賊試煉室，蹦出「你走進一間詭異的停屍間…」。

判準來自出貨資料：`1SS.DAT` 的類別 0 共四筆，**四筆的屬性都剛好是 `0x00`** ——
所以「屬性 0」是作者標「這一格沒事」的寫法。加了 `SpecialTile.Dead()` 擋掉。
> **陷阱解除（`traps.go` 的 `Consume`）那條路一直有同一個病**，
> 只是從來沒人踩回去看。
...
| 場景 | 結果 |
|---|---|
| 盜賊間（隊上有盜賊）| 「盜賊的試煉室。你身處一間**灰色**的房間…」＋ 嘲諷那句 → 過關 |
| 武僧間（隊上沒武僧）| 「隊伍裡沒有這個職業，直接過」 |
| 巫師間 | **只有 Wopple 上場**對一隻小惡魔 → 戰勝 → 「陣型還原成 `[0 ff 1 ff 2 ff 3 ff 4]`」→ 過關 |
| 遊俠間 | **只有 Menhir**（15/27）對 80 HP 的洞穴熊 → **戰敗**，陣型照樣還原，**不給旗標** |
| 遊俠間（Menhir 已死）| 「人都倒了，趕到 (42,19)」 |
| 過關的房間再走一次 | **完全沒反應**（`Dead()` 生效之前會蹦出停屍間那段） |
| 寶珠（十間沒全過）| 沒反應（原版也只放個音效） |
| 寶珠（`-proving`）| 「Wopple 第 2 格拿到，劇情階段 → 1」→ 紮營睡覺 → **馬利馮的預言播出來** |

最後一列把 §3 那條因果鏈整條跑通了：
**十間全過 → 寶珠 → `+0xb9 = 1` → 睡覺播預言。**

---

## 6. 對 worklist 的影響

- **A7 只剩 case 6**（附魔工坊 ＝ C2）。7／8／9 都接上了。
- 新增兩個存檔欄位：`+0x8a` 十間過關旗標、`+0xab` 現在在第幾間，
  以及 `+0x80` 的陣型備份（原版就放在 trailer 裡）。
- 新增偵錯旗標 `-proving`（十間都當成已過），用來驗寶珠那一格。
