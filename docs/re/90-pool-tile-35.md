...
## 5. 對引擎的影響（2026-07-27 已接上）

`TriggerHardBlock`／`tileHardBlock` 已改名為 `TriggerPool`／`tilePool`，
`docs/spec/03-events.md` 的閘門那一行也改了。實作在
`internal/game/pool.go` ＋ `cmd/demonwinter/poolui.go`。

攔截點在**移動之前**（`World.TileAhead`），因為原版的回傳碼發生在
寫座標之前 —— 隊伍轉了向但不會走上去。

實跑（地圖 3 的 `(14,48)`，全遊戲僅有的一格）：
Menhir `15/27` 連喝三口 `+1`／`+1`／`+4` → `21/27`，額度 `7 → 4`，
而隊伍座標從頭到尾停在 `(15,48)`。滿血的人喝下去顯示「回復了 0 點」，
**額度照樣扣一次** —— 原版的 `dec` 排在擲點之前。

## 6. 順帶一提：新遊戲喝不到

原版的新遊戲初始化**沒有寫 `+0xaa`**（全檔只有三處碰它，見 §2），
所以剛開局是 0 —— **第一次睡覺之前水池是乾的**。
