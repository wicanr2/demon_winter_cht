...
> **2026-07-29 補證**：`1000:11e5` 的 effect 12 已完整讀出：
> `light = clamp(SP*K/M, 1, 4)`，直接寫隊伍 `+0xa7`。因此照明效果公式已驗證；
> Magic Torch 的最低門檻仍以資料表 `M=2` 為原版執行檔行為。營地法術與魔法
> 物品的實作映射見 [`docs/re/114`](114-camp-spell-tail-and-magic-item-charges.md)。
