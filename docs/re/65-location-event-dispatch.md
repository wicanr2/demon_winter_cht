...
> ⚠ 本節原本寫「`ds:0x52f4` 是 tile 暫存（`docs/re/58`）」「`ds:0x52f6` 是個狀態變數，
> 誰在設還沒追」——**兩句都已推翻，見 `docs/re/83`**。
> 這一對是 `nSS.DAT` 查表函式 `222f:1321` 的 in/out 參數：
> 進去是座標，命中後 `ds:0x52f4` 變序號、`ds:0x52f6` 變 `attr & 0x1f`。
> 所以下表的 case 編號 = **`nSS.DAT` 類別 5 記錄的「值」**，觸發座標全部可查。
...
- ~~**新的未解**：`ds:0x52f6` 這個事件狀態變數誰在設~~
  → **已解，見 `docs/re/83`**：它不是狀態變數，是 `nSS.DAT` 查表函式
  `222f:1321` 的輸出參數，值 = 特殊格 attr 的低 5 bit。
  **類別 5 的記錄就是這張表的觸發格，值就是 case 編號。**
