...
- 新戰鬥單位（新的 `combat_record` 槽位）被寫入屬性的那一步——`FUN_138d_2f7e`、
  ...
  優先追）。
- 幻術每回合消失的機率已於 `docs/re/115` 結案：side 3／13 在輪到行動前
  執行 `Roll(10) < 3`，因 Roll 值域是 1–10，故為 **20%**。召喚單位
  建立當回合不能行動也已由 `docs/re/20` §9.1 結案：行動佇列在施法前
  已拍成快照，召喚函式不追加 `DS:5152/5154`，所以下回合才首次入列。
