...
第二波 `rebuild-battle-wave2.sh` 再完成 `0d/0e/0f/10/12/13/14/15`：
不死戰士、骷髏法師、腐敗土丘、幽魂、蜘蛛、鼠、蝙蝠與 Bugem。兩波合計
16 組、128 個方向／步態 frame；加上既有 Orc 單格後是 129/224。

第三波 `rebuild-battle-wave3.sh` 完成 `06/09/0a/11/16/17/18/19`：
Stalker、蟲群、鬼火、Orc 與火／金屬／冰／靈體元素。Orc 已由單格特例升級為
完整四向組，三波合計 24 組、192/224。

最終波 `rebuild-battle-wave4.sh` 完成 `1a/1b/1c/1d`：風元素、龍、
惡魔／Xeres 與巨人。28 個實際使用外觀的方向覆寫至此 224/224；同方向兩個
步態仍共用視圖，這是動畫 polish 缺口，不是方向或外觀缺格。
...
世界地圖 `0x01–0x0c` 是一套交錯森林，不是十二個無關圖示。原先已完成
`0x04/07/0b` 的單樹、雙樹與林緣；其餘九格由 `normal/winter-forest-suite.png`
各裁出不同冠層構圖，再以 `rebuild-forest-suite.sh` 建立共用邊界。這保留
密林、單樹與林緣的層級差異，也避免把一張森林圖複製到九個索引。

火山世界地圖 56 大量交錯 `0x2a/0x33`：前者是會遮擋戰場視線的黑岩峰，
