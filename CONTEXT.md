...
| 戰鬥單位 `+0x20` 是陣營欄位（2 怪物／11 玩家）| **這是我自己下太快的結論，同日收回。** 只看了 AI 目標檢查（`> 10`）與噴吐計數（`>= 11`）兩處就定案；第三處（AI 選法術的 `== 2`）會去讀隊伍角色的技能旗標，只對玩家側槽位成立，把極性反過來了。兩種讀法各自都能自圓其說 → 維持未解。**兩個相符的呼叫點不等於解出來了**，尤其當它們是同一個作者寫的同一套慣例 |
