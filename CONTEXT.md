...
| Go 引擎本體 | **M1–M5 完成**（見下），**開場標題畫面已接上**。原版版面刻意不復刻（中文需要 640×400）；**原版怪物 AI 已解到「挑目標」為止**（`FUN_138d_03f5` 決策樹、`FUN_138d_065e` 選法術與效果分派）：目標記憶、選法術、投入法力、依 K 正負挑敵我、範圍誤傷否決、附身術都已實作。**剩噴吐傷害**（範圍已解、傷害公式未讀）|
...
### 這一輪（2026-07-26）：怪物 AI 挑目標（`docs/re/23`）
...
驗收工具也補了兩樣：`-battle` 旗標（headless 截圖按不到 B 鍵）與
`DW_LOG=1`（戰鬥紀錄同步吐 stderr，畫面只留 8 行看不到完整時間軸）。
