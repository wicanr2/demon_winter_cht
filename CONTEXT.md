...
| Go 引擎本體 | **M1–M5 完成**（見下）。施法／道具選擇畫面、原版版面尚未實作 |
...
| M4 戰鬥 | ✅ 規則＋流程：武器骰表、戰鬥風格、行動順序、普通攻擊（命中／爆擊／傷害／毒／功夫暈眩）、四個動作、法術系統（參數表 43 筆／通式／即死／束縛／枯萎）、回合狀態機、勝負判定、召喚三槽、怪物進場擲點。端到端：踏到事件格 → 讀敘述 → 6 隻怪物開打 → 跨回合結算。**玩家指令選單已做**：行動點（初值＝速度）、走位與轉向、攻擊／驅散不死／祈禱／汲取法力／閃避／結束回合，熱鍵沿用原版。**施法與使用道具的選擇畫面、AOE、原版怪物 AI 未做** |
...
| 中文化 | 🔶 顯示層 + 事件敘述完成：倚天 16×15 點陣字（Big5 分區索引已驗證）、中英混排排版格與標點禁則、畫布 640×400、viewer 全介面中文、**五個 `DATA*.TXT` 共 105 筆事件敘述 100% 翻完**並有自動品質閘（`dwstrings check`）。**未做**：`DEMON.INT` 的 800 條 UI 文案只抽了清單（重製版不讀它，是參考不是資源）、城鎮／建角／戰鬥指令等尚未實作的畫面自然也還沒有文案 |
