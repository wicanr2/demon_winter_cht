...
| 項目 | 狀態 |
|---|---|
| ~~即死／束縛／枯萎法術的判定邏輯~~ | ✅ **已解**（`docs/re/18` §7，公式已寫入 `docs/spec/02-combat.md`）。搭配 `docs/re/15` 的 K/M 常數表，三類法術可實作 |
| 召喚生物機制 | 未解（`effect_type 8/9` 在 Cast 與 Use 路徑行為不一致）。**唯一擋住戰鬥規格升 READY 的項目**。已找到更近的入口 `FUN_138d_34d6`（`docs/re/18` §8.1） |
| `OPEN.PIE` | 未解（102,160 B，不符 3.5× 規律） |
| 城鎮經濟 | ~~程式碼落在 Ghidra 未發現區域~~ **大幅推進**（`docs/re/19`）：神殿／學院／碼頭／治療所／酒館五項有完整公式；市集議價與說服技能介入方式仍未解 |
| 一天幾小時（26 vs 38） | DOSBox 實測未完成（隊伍被困小中庭） |
| 種族存在存檔哪裡 | 原以為是 `0x0c`，已推翻，位置未知 |
| ~~第三張跳表~~ | ✅ **已修復**（`docs/re/18`）：函式 361→394、警告 269→86。全檔掃描找到 21 張同類跳表，**20 張標註成功、1 張誠實跳過**（`25be:139a`，真實控制流糾纏，非處理順序問題） |
...
| [`02-combat.md`](docs/spec/02-combat.md) | **DRAFT** — 戰鬥（框架／普通攻擊／一般法術／即死・束縛・枯萎可用；**只剩召喚・幻術未解**） |
...
| [`19-town-economy.md`](docs/re/19-town-economy.md) | **城鎮經濟**：神殿／學院／碼頭／治療所／酒館公式；角色記錄 6 個新欄位 |
...
1. **召喚／幻術（`effect_type 8/9`）** — 戰鬥規格升 READY 的唯一障礙。
   入口 `FUN_138d_3161`（12 生物選單，成本在 `0x51d4+idx*0x16+0x10`）已找到，
   下一步展開 `FUN_138d_34d6`（`docs/re/18` §8.1）
2. **市集議價與說服技能** — `docs/re/19` §7.3 只解出骨架；`FUN_278d_22bc` 那 2000+ bytes
   還有後半段沒讀。或改用 DOSBox 動態測試（同商品反覆議價記錄價格）
3. **治療所四個費率常數的來源** — 由呼叫端傳入，呼叫點未找到（`docs/re/19` §10 第 3 項）
4. **種族存在存檔哪裡** — 原以為 `0x0c`，已推翻，建角流程缺一塊
5. **把 `docs/re/19` §8 的 6 個角色欄位合併進 `docs/formats/game-data-tables.md`**
   （含更正：`+0x102` 是列舉值不是位元旗標；`+0xf6` 解決「職業完全未找到」）
6. 補齊規格：事件觸發、移動、建角升級、**城鎮經濟**（證據等級已夠，尚未收攏成 spec）

**引擎實作要等規格齊。** 這是刻意的選擇，避免邊做邊猜。
