...
| 中文化 | 🔶 顯示層 + 事件敘述完成：倚天 16×15 點陣字（Big5 分區索引已驗證）、中英混排排版格與標點禁則、畫布 640×400、viewer 全介面中文、**事件敘述 105 筆 + 法術名 43 筆共 148 條 100% 翻完**並有自動品質閘（`dwstrings check`）。**未做**：`DEMON.INT` 的 800 條 UI 文案只抽了清單（重製版不讀它，是參考不是資源）、城鎮／建角／戰鬥指令等尚未實作的畫面自然也還沒有文案 |
