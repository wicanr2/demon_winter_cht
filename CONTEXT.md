...
| Go 引擎本體 | **M1／M2／M3 完成**（見下）。戰鬥、城鎮、建角 UI、原版版面尚未實作 |
...
| M4 戰鬥 | 🔶 規則完成：武器骰表 14 項、戰鬥風格、行動順序（三個排除條件＋穩定降冪）、普通攻擊（命中／爆擊／傷害／毒／功夫暈眩）、驅散不死／閃避／祈禱／汲取、法術參數表 43 筆、效果通式、即死／束縛施加與解除／枯萎。**戰鬥流程主迴圈與 UI 未做** |
