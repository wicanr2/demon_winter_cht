...
| Go 引擎本體 | **M1–M5 完成**（見下）。使用道具選擇畫面、AOE、原版版面尚未實作 |
...
| M2 事件與文字 | ✅ 觸發閘門（tile 0x11/0x53 查表、五個 tile 直接當索引、0x35 硬阻擋）、`EXITS.DAT` 掃描與事件索引累計、`DATA*.TXT` 文字顯示（中文化後改為 38 格逐字／逐詞斷行＋標點禁則，仍是每 5 行翻頁）。端到端：走進 (54,16) 跳出「骸骨廳。地上散落著數百具頭骨與骸骨……」並可翻頁。**傳送尚未接** |
...
| M4 戰鬥 | ✅ 規則＋流程：武器骰表、戰鬥風格、行動順序、普通攻擊（命中／爆擊／傷害／毒／功夫暈眩）、四個動作、法術系統（參數表 43 筆／通式／即死／束縛／枯萎）、回合狀態機、勝負判定、召喚三槽、怪物進場擲點。端到端：踏到事件格 → 讀敘述 → 6 隻怪物開打 → 跨回合結算。**玩家指令選單 + 施法選單已做**：行動點（初值＝速度）、走位與轉向、攻擊／施法／驅散不死／祈禱／汲取法力／閃避／音效開關／結束回合，熱鍵沿用原版。施法選單列 43 筆法術（中文名，最低法力對上 RE 表），通式數值增減與四類特殊判定都會生效。**未做**：AOE、投入點數輸入框（一律全投）、游標選目標（傷害對正前方、治療對自己）、使用道具、原版怪物 AI |
